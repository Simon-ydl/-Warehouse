...
var RELOGIN_URL = HOST_PATH + 'cas/registerLogin';
...
{
    var userInfo;
    globalSyncPost(WEB_ROOT + '/authUser/readSelfFromCache',{},function(result)
    {
        if(result.data) {
...
function areacodeIsSzbm(areacode)
{
    if('441900,DGS919,DGS920,DGS921,DGS922,DGS923'.indexOf(areacode) != -1)
    {
        return true;
    }
    else {
    ...
    var mask = layui.layer.msg('处理中，请稍候...',
        {
            ...
            time: 100 * 1000,
            success: function () {
                globalPost(url,params,true,'application/json',function(result)
                {
                    if(successCallBack) successCallBack(result);
                },function(){layui.layer.close(mask);});
...
{//按规则设置键值到表单中
    ...
    if(hasValue && el.length > 0) {
        if(el.is('select'))
            el.val(value);
        else if(el.is('input[type="text"]') || el.is('input[type="hidden"]') || el.is('textarea'))
        ...
        if(el.length > 0) {
            if(hasValue && el.is('select'))
                el.val(value);
            if(hasValue
                && (el.is('input[type="text"]')|| el.is('input[type="hidden"]') || el.is('textarea')))
            {el[0].defaultValue = value;if($.browser.msie) el.val(value);}
